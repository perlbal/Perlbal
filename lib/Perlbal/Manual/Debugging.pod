...
You can control the amount of debugging messages Perlbal dumps by setting the environment variable C<PERLBAL_DEBUG> to a value between 0 and 4:
...
=item * If we're not sending keep-alive header information back to the client, L<Perlbal::ClientHTTPBase> prints C<  doing connection: close>
