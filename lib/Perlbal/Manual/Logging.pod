...
=item * When deciding whether we should spawn one or more backend connections, if the total of pending connections is negative, L<Perlbal::Service> will log a critical message:
